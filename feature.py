...
class FeatureExtraction:
    ...
    def __init__(self, url):
        ...
        try:
            ...
            # FIX: was `response.text` (missing self.)
    ...
    def Favicon(self):
        try:
            for head in self.soup.find_all('head'):
                # FIX: was `domain` (undefined bare variable), use self.domain
                for link_tag in self.soup.find_all('link', href=True):
                    dots = [x.start(0) for x in re.finditer(r'\.', link_tag['href'])]
                    if self.url in link_tag['href'] or len(dots) == 1 or self.domain in link_tag['href']:
                        return 1
            return -1
        except Exception:
            return -1
    ...
    def RequestURL(self):
        try:
            # FIX: success and i were never initialised
            i, success = 0, 0
    ...
    def InfoEmail(self):
        try:
            # FIX: was `self.soap` (typo) → self.response.text; also fixed regex
    ...
    def WebsiteTraffic(self):
        try:
            # FIX: bare `url` → self.url; Alexa API is dead — returns -1 gracefully
            rank_data = urllib.request.urlopen(
                "http://data.alexa.com/data?cli=10&dat=s&url=" + self.url, timeout=5
            ).read()
    ...
    def PageRank(self):
        try:
            ...
            # FIX: was `rank_checker_response` (undefined) → prank_checker_response
    ...
    def GoogleIndex(self):
        try:
            # FIX: googlesearch API changed — search() returns a generator, wrap in list
